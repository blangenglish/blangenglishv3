...
function EmpresaStudentsTab() {
  ...
  const loadStudents = useCallback(async () => {
    ...
    try {
      ...
      if (error || !data?.students) {
        ...
        setStudents((data.students).filter(s => s.is_empresa === true));
      }
    } catch (e) {
      console.error('[EmpresaStudentsTab] loadStudents error:', e);
      setStudents([]);
    } finally {
      setLoading(false);
    }
  ...
  const loadPayHistory = async (studentId, force = false) => {
    ...
    try {
      ...
      console.error('[EmpresaStudentsTab] loadPayHistory error:', e);
    } finally {
  ...
  const handleActivate = async () => {
    ...
    try {
      ...
      const fmtShort = (d) => d.toLocaleDateString('es-CO', { day: '2-digit', month: 'short', year: 'numeric' });
  ...
  /* ── Detail view ── */
  if (selected) {
    const status = STATUS_LABELS[selected.account_status] || STATUS_LABELS.pending;
      ...
      <div>
        ...
        <div className="bg-card border border-border rounded-2xl p-5 mb-4">
          <h3 className="font-bold text-sm mb-4 flex items-center gap-2">
            <CreditCard className="w-4 h-4 text-teal-600" /> Activar {PLAN.name}
          </h3>
              ...
              {['PayPal', 'Bold (PSE)', 'Transferencia', 'Efectivo'].map(m => (
                <button
                  ...
                  className={`flex-1 py-1.5 rounded-lg text-xs font-bold border transition-all ${
                    activationForm.method === m ? 'bg-teal-600 border-teal-600 text-white' : 'border-border text-muted-foreground hover:border-teal-400'
                  }`}
                >{m}</button>
              ))}
            </div>
    ...
    <div>
      ...
      <div className="flex items-center gap-4 mb-5">
        ...
        <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-2 text-sm">
          <span className="font-bold text-green-800">{students.filter(s => s.account_status === 'active').length}</span>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-2 text-sm">
          <span className="font-bold text-amber-800">{students.filter(s => s.account_status === 'pending_payment').length}</span>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="font-medium">{search ? 'No se encontraron resultados' : 'Aún no hay estudiantes de empresa'}</p>
          {!search && <p className="text-sm mt-1">Agrega el primer estudiante con el botón de arriba</p>}
        </div>
      ) : (
        <div className="bg-card border border-border rounded-2xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border/40">
              {filtered.map(s => {
                const status = STATUS_LABELS[s.account_status] || STATUS_LABELS.pending;
                return (
                  <tr key={s.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3 hidden sm:table-cell text-muted-foreground">{s.company_name || '—'}</td>
                    <td className="px-4 py-3 hidden md:table-cell text-muted-foreground">{s.created_at ? fmtDate(s.created_at) : '—'}</td>
                    <td className="px-4 py-3">
                      <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${status.cls}`}>{status.label}</span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <Button variant="ghost" size="sm" onClick={() => handleSelect(s)} className="rounded-lg text-xs font-semibold">
                        Ver detalles →
                      </Button>
                    </td>
                  </tr>
...
function AddStudentModal({ onClose, onCreated }) {
  const [form, setForm] = useState({ name: '', email: '', company: '', password: '', level: 'B1' });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data: authResult, error: authErr } = await supabase.functions.invoke('admin-user-manager-2026', {
        body: { action: 'create', email: form.email.trim(), password: form.password.trim(), full_name: form.name.trim() },
        headers: session?.access_token ? { Authorization: `Bearer ${session.access_token}` } : {},
      });
      ...
      supabase.functions.invoke('send-session-email', {
        body: { type: 'empresa_credenciales', studentName: form.name.trim(), studentEmail: form.email.trim(), password: form.password.trim(), company: form.company.trim(), level: form.level },
      }).catch(() => {});
      onCreated();
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative bg-background rounded-3xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          <p className="text-sm text-muted-foreground mb-5">Se creará la cuenta y se enviará el correo con credenciales</p>

          <div className="space-y-4">
            <div className="space-y-1.5">
              <Label className="text-sm font-medium">Nombre completo *</Label>
              <Input placeholder="Nombre completo" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} className="rounded-xl" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-sm font-medium">Correo electrónico *</Label>
              <Input type="email" placeholder="correo@empresa.com" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} className="rounded-xl" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-sm font-medium">Empresa</Label>
              <Input placeholder="Nombre de la empresa (opcional)" value={form.company} onChange={e => setForm(p => ({ ...p, company: e.target.value }))} className="rounded-xl" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-sm font-medium">Contraseña temporal *</Label>
              <div className="relative">
                <Input type={showPass ? 'text' : 'password'} placeholder="Mínimo 6 caracteres" value={form.password} onChange={e => setForm(p => ({ ...p, password: e.target.value }))} className="rounded-xl pr-10" />
              ...
              <div className="flex gap-2">
                {['B1', 'B2', 'C1'].map(l => (
                  <button key={l} type="button" onClick={() => setForm(p => ({ ...p, level: l }))}
                    className={`flex-1 py-2 rounded-xl border-2 font-bold text-sm transition-all ${form.level === l ? 'bg-teal-600 border-teal-600 text-white' : 'border-border text-muted-foreground hover:border-teal-400'}`}
                  >{l}</button>
                ))}
              </div>
            </div>
            {error && <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-sm text-red-700">{error}</div>}
            <Button className="w-full bg-teal-600 hover:bg-teal-700 text-white rounded-xl font-bold py-6"
            ...
            >
              {loading
                ...
                : <span className="flex items-center gap-2"><Send className="w-4 h-4" /> Crear estudiante y enviar credenciales</span>
              }
            </Button>
          </div>
        </div>
...
function EmpresaModulesTab() {
  ...
  const [msg, setMsg] = useState(null);
  const [form, setForm] = useState({ title: '', level: 'B1', description: '', content: '', is_published: true, sort_order: 0 });
  ...
  const loadModules = useCallback(async () => {
    ...
    try {
      ...
      console.error('[EmpresaModulesTab] error:', e);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setForm({ title: '', level: 'B1', description: '', content: '', is_published: true, sort_order: 0 });
      setEditingId(null);
      setShowForm(false);
      await loadModules();
    } catch (e) {
  ...
  const handleEdit = (mod) => {
    setForm({ title: mod.title, level: mod.level, description: mod.description || '', content: mod.content || '', is_published: mod.is_published, sort_order: mod.sort_order });
    setEditingId(mod.id);
    setShowForm(true);
  };
  ...
  const handleTogglePublish = async (mod) => {
    try {
      await adminUpdate('empresa_modules', { is_published: !mod.is_published, updated_at: new Date().toISOString() }, mod.id);
  ...
  return (
    <div>
        ...
        <div className="flex gap-1 bg-muted rounded-xl p-1">
          {['all', 'B1', 'B2', 'C1'].map(l => (
            <button key={l} onClick={() => setFilterLevel(l)}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${filterLevel === l ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}
      ...
      {showForm && (
        <div className="bg-card border border-teal-200 rounded-2xl p-5 mb-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
            <div className="space-y-1.5">
              ...
              <Input placeholder="Ej: Business English Meetings" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} className="rounded-xl" />
            </div>
            <div className="space-y-1.5">
          ...
          <div className="space-y-1.5 mb-3">
            ...
            <Input placeholder="Breve descripción del módulo" value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} className="rounded-xl" />
          </div>
          <div className="space-y-1.5 mb-3">
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="space-y-1.5">
              ...
              <Input type="number" min="0" value={form.sort_order} onChange={e => setForm(p => ({ ...p, sort_order: Number(e.target.value) }))} className="rounded-xl" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-medium">¿Publicado?</Label>
              <button type="button" onClick={() => setForm(p => ({ ...p, is_published: !p.is_published }))}
                className={`w-full py-2 rounded-xl border-2 font-bold text-sm transition-all ${form.is_published ? 'bg-green-600 border-green-600 text-white' : 'border-border text-muted-foreground'}`}
              >{form.is_published ? '✅ Publicado' : '🔒 Oculto'}</button>
            </div>
          </div>
          <div className="flex gap-2">
            <Button onClick={handleSave} disabled={!form.title.trim()} className="bg-teal-600 hover:bg-teal-700 text-white rounded-xl font-bold">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map(mod => (
            <div key={mod.id} className="bg-card border border-border rounded-2xl p-4 flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                  <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full border ${LEVEL_COLORS[mod.level] || 'bg-muted text-muted-foreground border-border'}`}>{mod.level}</span>
                  {!mod.is_published && <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full font-medium">Oculto</span>}
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => handleTogglePublish(mod)} className="p-2 rounded-lg text-muted-foreground hover:bg-muted transition-colors" title={mod.is_published ? 'Ocultar' : 'Publicar'}>
                  {mod.is_published ? <ToggleRight className="w-5 h-5 text-green-600" /> : <ToggleLeft className="w-5 h-5" />}
                </button>
                <button onClick={() => handleEdit(mod)} className="p-2 rounded-lg text-muted-foreground hover:bg-muted transition-colors">
                  <Edit2 className="w-4 h-4" />
                </button>
                <button onClick={() => handleDelete(mod.id)} className="p-2 rounded-lg text-muted-foreground hover:bg-red-50 hover:text-red-600 transition-colors">
